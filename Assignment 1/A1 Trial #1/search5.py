...
def greedy_best_first_search(grid, start, goals, heuristic):
    ...
    priority_queue = [(heuristic(start, goal), Node(start[0], start[1])) for goal in goals] 
    ...
    while priority_queue:
        ...
        for dx, dy in [(0, 1), (-1, 0), (0, -1), (1, 0)]:  # Up, Left, Down, Right
            next_x, next_y = current.x + dx, current.y + dy
            if (next_x, next_y) not in visited and grid.is_valid(next_x, next_y):
                for goal in goals:  # Iterate over remaining goals
...
def a_star_search(grid, start, goals, heuristic):
    # Include cost to reach the goal and cost to reach this node
    priority_queue = [(0, 0, Node(start[0], start[1]))]  
    heapq.heapify(priority_queue)
    visited = set()
    while priority_queue:
        ...
        for dx, dy in [(0, 1), (-1, 0), (0, -1), (1, 0)]:  # Up, Left, Down, Right
            ...
            if (next_x, next_y) not in visited and grid.is_valid(next_x, next_y):
                for goal in goals:  # Iterate over remaining goals
                    heapq.heappush(priority_queue, (heuristic(start, goal) + heuristic((next_x, next_y), goal), heuristic(start, goal) + heuristic((next_x, next_y), goal), Node(next_x, next_y, current)))
                visited.add((next_x, next_y))
    return []

# Custom heuristic function (Manhattan Distance) - set for multiple goal targets
'''def heuristic(current, goals):
    if isinstance(goals, tuple):   # Single goal node
        return abs(current[0] - goals[0]) + abs(current[1] - goals[1])
    else:                          # Set of goal nodes (especially for more than 2 targets)
